...
const Chatbot = () => {
  ...
  const [history, setHistory] = useState([
    { role: 'assistant', content: 'Hi there! I am the Weybee assistant. I can answer questions about the company. How can I help you today?' }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await axios.post(`${API_URL}/chatbot`, {
        ...
        history: newHistory.slice(-6).map(h => ({ role: h.role, content: h.content }))
      });
      
      setHistory([...newHistory, { role: 'assistant', content: response.data.answer }]);
    } catch (error) {
      setHistory([...newHistory, { 
        role: 'assistant', 
